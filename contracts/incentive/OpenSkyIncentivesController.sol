...
import '../tokenomic/libraries/types/DistributionTypes.sol';
import '../tokenomic/libraries/utils/VersionedInitializable.sol';
import '../tokenomic/libraries/utils/DistributionManager.sol';
import '../tokenomic/stake/IStakedToken.sol';

/**
 * @title OpenSkyIncentivesController
 * @notice Distributor contract for rewards to the OpenSky protocol
 * @author OpenSky Labs
 **/
contract OpenSkyIncentivesController is
    IOpenSkyIncentivesController,
    ...
    DistributionManager
{
    ...
    function getRewardsBalance(address[] calldata assets, address user) external view override returns (uint256) {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (userState[i].stakedByUser, userState[i].totalStaked) = IOpenSkyOToken(assets[i]).getScaledUserBalanceAndSupply(
                user
            );
        }
