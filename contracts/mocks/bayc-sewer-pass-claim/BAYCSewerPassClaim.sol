...
contract BAYCSewerPassClaim is Operator, YugaVerify, ReentrancyGuard {
    ...
    uint256 constant NO_DOGGO = 10000; // BAKC  tokenId不存在
     ...
     */
    function claimBaycBakc(uint256 baycTokenId, uint256 bakcTokenId) external claimable nonReentrant {
     ...
     * @param bakcTokenId token id of the dog
     */
    function claimMaycBakc(uint256 maycTokenId, uint256 bakcTokenId) external claimable nonReentrant {
    ...
    function _checkBaycClaim(uint256 tokenId) private {
        if (!verifyTokenOwner(BAYC_CONTRACT, tokenId)) revert UnauthorizedOwner();
    ...
    function _checkMaycClaim(uint256 tokenId) private {
        if (!verifyTokenOwner(MAYC_CONTRACT, tokenId)) revert UnauthorizedOwner();
    ...
    function _checkBakcClaim(uint256 tokenId) private {
        if (!verifyTokenOwner(BAKC_CONTRACT, tokenId)) revert UnauthorizedOwner();
    ...
    function _mintSewerPass(
        ...
        uint256 sewerPassTokenId = sewerPassContract.mintSewerPass(_msgSender(), mintData);
        emit SewerPassMinted(sewerPassTokenId, tier, baycMaycTokenId, bakcTokenId);
    }
