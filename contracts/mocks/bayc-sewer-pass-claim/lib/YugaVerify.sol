...
contract YugaVerify {
    ...
    constructor(address _warmWalletContract, address _delegateCashContract) {
        if (_warmWalletContract == address(0) || _delegateCashContract == address(0)) revert ZeroAddressCheck();
     ...
     */
    function verifyTokenOwner(address tokenContract, uint256 tokenId) internal view returns (bool) {
        ...
        return (msg.sender == tokenOwner ||
            msg.sender == WarmInterface(WARM_WALLET_CONTRACT).ownerOf(tokenContract, tokenId) ||
