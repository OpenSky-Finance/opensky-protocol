...
contract BAYCSewerPass is ERC721EnumerableMod, Operator {
     ...
     * 192          128           64     0
     */
    function mintSewerPass(address to, uint256 mintData) external returns (uint256) {
     ...
     */
    function tokenIdsByWallet(address _owner) external view returns (uint256[] memory) {
     ...
     */
    function toggleMinterContract(address _minterContract) external onlyOperator {
        if (contractIsLocked) revert ContractIsLocked();
     ...
     */
    function setNftLicenseTerms(string memory _nftLicenseUri) external onlyOperator {
     ...
     */
    function setRegistryAddress(address _registryAddress) external onlyOperator {
