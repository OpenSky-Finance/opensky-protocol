...
describe('borrow', function () {
    ...
    beforeEach(async () => {
        ...
        const { WBTC, OpenSkySettings, OpenSkyNFT, OpenSkyPool, user001, user002 } = ENV;

        await WBTC.mint(user001.address, '5000000000');
        await WBTC.mint(user002.address, '3000000000');

        await user001.WBTC.approve(OpenSkyPool.address, '5000000000');
        await user002.WBTC.approve(OpenSkyPool.address, '3000000000');

        await user001.OpenSkyPool.deposit(4, '100000000', user001.address, 0);
        await user002.OpenSkyPool.deposit(4, '100000000', user002.address, 0);

        await OpenSkySettings.addToWhitelist(
            4,
            OpenSkyNFT.address,
            'OpenSky NFT', 'ONFT', '5000',
            ONE_YEAR, ONE_YEAR, 3 * 24 * 3600, 1 * 24 * 3600
        );
    });

    it('user borrow 1.5 WBTC', async function () {
        const { OpenSkyDataProvider, OpenSkyNFT, WBTC, OpenSkyLoan, OpenSkySettings, borrower } = ENV;

        let wbtcBalanceBeforeBorrow = await WBTC.balanceOf(borrower.address);
        let amount = parseUnits('1.5', 8);
        ...
        let wbtcBalanceAfterBorrow = await WBTC.balanceOf(borrower.address);

        // check staker ETH Balance
        expect(wbtcBalanceBeforeBorrow.add(amount)).to.be.equal(wbtcBalanceAfterBorrow);
        ...
        expect(loan.amount).to.be.equal(parseUnits('1.5', 8));
        expect(loan.borrowDuration).to.be.equal(365 * 24 * 3600);
        expect(LOAN_STATUS[loan.status]).to.be.equal('BORROWING');

        // check borrow rate
        expect(loan.borrowRate).to.be.equal(await OpenSkyDataProvider.getBorrowRate(4, 0, 0, 0, 0));
    });

    it('user borrow max eth if borrowLimit < availableLiquidity', async function () {
        const { OpenSkyDataProvider, OpenSkyNFT, MoneyMarket, WBTC, OpenSkyLoan, OpenSkyOToken, OpenSkyPool, borrower, user002 } = ENV;

        await user002.WBTC.approve(OpenSkyPool.address, parseUnits('2', 8));
        await user002.OpenSkyPool.deposit(4, parseUnits('2', 8), user002.address, 0);

        const availableLiquidity = await MoneyMarket.getBalance(WBTC.address, OpenSkyOToken.address);
        const borrowLimit = await OpenSkyPool.getBorrowLimitByOracle(4, OpenSkyNFT.address, 1);
        ...
        expect(await borrower.OpenSkyPool.borrow(4, borrowLimit, ONE_YEAR, OpenSkyNFT.address, 1, borrower.address));
    ...
    it('user borrow max eth if borrowLimit > availableLiquidity', async function () {
        ...
        await expect(
            borrower.OpenSkyPool.borrow(4, borrowLimit, ONE_YEAR, OpenSkyNFT.address, 1, borrower.address)
