...
export const __setup = deployments.createFixture(async () => {
    ...
    // ///////////////////////////////////////////////////////////////////
    // settings for incentive
    const aaveToken= await ethers.getContract('AaveToken')
    ENV.aaveToken =aaveToken
    ENV.rewardsVault = ENV.user005

    const {OpenSkyPoolIncentivesControllerLender,  rewardsVault} = ENV
    await OpenSkyPoolIncentivesControllerLender.initialize(rewardsVault.address)

    // adapt cases from aave
    ENV.pullRewardsIncentivesController = ENV.OpenSkyPoolIncentivesControllerLender
    ENV.aDaiBaseMock = ENV.aDAI
