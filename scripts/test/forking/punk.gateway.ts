...
describe('punk-gateway borrow', function () {
    ...
    beforeEach(async () => {
        ENV = await __setup();
        const { OpenSkyERC20Pool, OpenSkyPunkGateway, CryptoPunksMarket, borrower, user001, user002 } = ENV;
    ...
    it('borrow WETH successfully', async function () {
        ...
        let tx = await borrower.OpenSkyPunkGateway.borrow(1, BORROW_AMOUNT, 28 * 24 * 3600, PUNK_INDEX);
    ...
    it('borrow ETH successfully', async function () {
        ...
        const PunkBalanceBeforeTx = await CryptoPunksMarket.balanceOf(borrower.address);
        const WPunkBalanceBeforeTx = await CryptoPunksMarket.balanceOf(WrappedPunk.address);
        const tx = await borrower.OpenSkyPunkGateway.borrowETH(1, BORROW_AMOUNT, 28 * 24 * 3600, PUNK_INDEX);
...
describe('punk-gateway reapy', function () {
    ...
    beforeEach(async function () {
        ...
        await borrower.OpenSkyPunkGateway.borrow(1, ENV.BORROW_AMOUNT, 28 * 24 * 3600, ENV.PUNK_INDEX);

        ENV.LOAN_ID = 1;
    });

    it('borrow repay ETH from gateway, if penalty == 0', async function () {
        const { CryptoPunksMarket, OpenSkyLoan, PUNK_INDEX } = ENV;
        const { borrower } = ENV;
        const INFO: any = {};
        ...
        INFO.owner_of_punk0_after_repayed = await CryptoPunksMarket.punkIndexToAddress(PUNK_INDEX);
    ...
    it('borrow repay ETH from gateway, if penalty > 0', async function () {
        const { CryptoPunksMarket, OpenSkyLoan, LOAN_ID, PUNK_INDEX } = ENV;
        const { nftStaker } = ENV;
        const INFO: any = {};

        advanceTimeAndBlock(28 * 24 * 3600 + 100 * 3600);
