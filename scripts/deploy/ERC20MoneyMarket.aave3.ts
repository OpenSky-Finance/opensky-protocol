...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const aaveAddress = config.contractAddress.AAVE_V3_POOL;
    if (!aaveAddress) {
        throw new Error(`AAVE_V3_POOL address is not provided in network ${network}`);
    }

    // money market
    let AaveMoneyMarket = await deploy('AaveV3MoneyMarket', {
        from: deployer,
        args: [aaveAddress],
        log: true,
    });
...
export default func;
func.tags = ['ERC20MoneyMarket.aave3'];
